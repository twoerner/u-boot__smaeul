...
static struct bl808_clk_data bl808_glb_clks[] = {
	[CLK_CPU] = {
		.name		= "CPU",
...
static struct bl808_clk_data bl808_hbn_clks[] = {
	...
	[CLK_HBN_UART] = {
		.name		= "HBN_UART",
		.parents	= PRNTS(CLK_HBN_UART_SEL2,
					CLK_XCLK),
		.sel_reg	= HBN_GLB_OFFSET,
...
static struct bl808_clk_data bl808_mm_glb_clks[] = {
	[CLK_MM_UART] = {
		.name		= "MM_UART",
	...
	[CLK_MM_SPI] = {
		.name		= "MM_SPI",
		.parents	= PRNTS(FW_MM_MUXPLL_160M,
					CLK_MM_XCLK),
		.sel_reg	= MM_GLB_MM_CLK_CTRL_CPU_OFFSET,
	...
	[CLK_MM_MUXPLL] = {
		.name		= "MM_MUXPLL",
		.parents	= PRNTS(FW_MM_MUXPLL_240M,
					FW_MM_MUXPLL_320M,
					FW_TOP_CPUPLL_400M,
					FW_TOP_CPUPLL_400M),
		.sel_reg	= MM_GLB_MM_CLK_CTRL_CPU_OFFSET,
...
static struct bl808_clk_data bl808_pds_clks[] = {
	[CLK_PDS_PLL] = {
		.name		= "PDS_PLL",
		.parents	= PRNTS(FW_TOP_CPUPLL_400M,
					FW_TOP_AUPLL_DIV1,
					FW_TOP_WIFIPLL_240M,
					FW_TOP_WIFIPLL_320M),
...
static struct bl808_clk_data *
bl808_clk_get_data(struct bl808_clk_plat *plat, struct clk *clk)
{
...
static u32 bl808_clk_count_parents(struct bl808_clk_data *data)
{
	if (!data->sel_mask)
		return 1;
...
static int bl808_clk_resolve_parent(struct udevice *dev,
				    struct bl808_clk_data *data, u32 sel,
				    struct clk *out)
{
	struct clk parent = {};
	u8 id;
...
{
	struct bl808_clk_plat *plat = dev_get_plat(clk->dev);
	struct bl808_clk_data *data = bl808_clk_get_data(plat, clk);
...
{
	const struct bl808_clk_desc *desc =
		(const struct bl808_clk_desc *)dev_get_driver_data(dev);

	// TODO: move this out of the driver data
	...
	if (desc->bind_sysreset)
		device_bind_driver_to_node(dev->parent, "bl808_sysreset",
					   dev->name, dev_ofnode(dev), NULL);
...
static int bl808_clk_probe(struct udevice *dev)
{
	...
	for (size_t id = 0; id < desc->num_clks; ++id) {
		...
		if (IS_ERR(parent))
			...
			struct bl808_clk_plat *pplat = dev_get_plat(parent->dev);
			const struct bl808_clk_desc *pdesc = pplat->desc;
			struct bl808_clk_data *pdata = &pdesc->clks[parent->id];
		...
		printf("%2d %16s %16s %10ld %3d %3d %2d\n",
		       id, data->name, parent_name,
		       clk_get_rate(&clk),
		       data->sel_mask ? (readl(plat->base + data->sel_reg) & data->sel_mask) / (data->sel_mask & -data->sel_mask) : -1,
		       data->div_mask ? (readl(plat->base + data->div_reg) & data->div_mask) / (data->div_mask & -data->div_mask) : -1,
		       data->en_mask  ? (readl(plat->base + data->en_reg)  & data->en_mask)  / (data->en_mask  & -data->en_mask)  : -1);
	}
	printf("\n");
#endif
