...
{
	const struct bflb_pinctrl_desc *desc = dev_get_priv(dev);

	return desc->num_pins;
}
						   ...
						   uint func_selector)
{
	...
	return desc->functions[func_selector];
}
...
{
	// FIXME: Fix bind order and use dev_get_driver_data()
	dev_set_priv(dev, (void *)&bl808_pinctrl_desc);

	device_bind_driver_to_node(dev, "bflb_gpio", dev->name,
				   dev_ofnode(dev), NULL);
